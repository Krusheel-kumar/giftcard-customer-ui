...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans text-richBlack overflow-x-hidden relative selection:bg-[#710000] selection:text-cream">
      ...
      <main className="relative z-10 flex flex-col items-center justify-center min-h-[100dvh] w-full pt-16 md:pt-20 pb-4 md:pb-0">
        <AnimatePresence mode="wait">
          ...
          {step === 'mobile' && (
            <motion.div key="mobile" variants={fadeUp} initial="hidden" animate="visible" exit="exit" className="w-full max-w-md px-4 relative z-10 mt-auto md:mt-0 mb-4 md:mb-0">
              <div className="bg-white/70 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(113,0,0,0.1)] border border-white p-6 md:p-10 text-center relative overflow-hidden">
                {/* Subtle top gradient line */}
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-[#710000]/40 to-transparent" />
                
                <img src={emblem} className="absolute -top-12 -right-12 w-40 md:w-56 opacity-[0.03] pointer-events-none rotate-12" />
                
                <h2 className="font-serif text-2xl md:text-3xl font-bold mb-2 md:mb-3 text-[#710000]">Send Rakhi Love</h2>
                <p className="text-gray-500 mb-6 md:mb-8 font-medium text-xs md:text-sm px-2 md:px-4">Enter your details to generate your beautifully crafted Rakhi Gift Card.</p>
                
                <form onSubmit={handleSendOtp} className="space-y-4 md:space-y-5 text-left relative z-10">
                  <div>
                    <label className="block text-[10px] md:text-xs font-bold tracking-widest uppercase mb-1.5 md:mb-2 px-1 text-gray-500">Your Name</label>
                    <input required type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="e.g. Rahul Sharma"
                      className="w-full bg-white/80 px-4 md:px-5 py-3.5 md:py-4 rounded-2xl border border-gray-200 focus:bg-white focus:border-[#710000] focus:ring-4 focus:ring-[#710000]/10 outline-none transition-all font-bold text-base md:text-lg shadow-sm" />
                  </div>
                  <div>
                    <label className="block text-[10px] md:text-xs font-bold tracking-widest uppercase mb-1.5 md:mb-2 px-1 text-gray-500">Mobile Number</label>
                    <div className="relative">
                      <span className="absolute left-4 md:left-5 top-1/2 -translate-y-1/2 text-gray-400 font-bold border-r border-gray-200 pr-2 md:pr-3">+91</span>
                      <input required type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value.replace(/\D/g, ''))} maxLength={10} placeholder="99999 99999"
                        className="w-full bg-white/80 pl-16 md:pl-20 pr-4 md:pr-5 py-3.5 md:py-4 rounded-2xl border border-gray-200 focus:bg-white focus:border-[#710000] focus:ring-4 focus:ring-[#710000]/10 outline-none transition-all font-bold text-base md:text-lg shadow-sm tracking-wide" />
                    </div>
                  </div>
          ...
          {step === 'otp' && (
            <motion.div key="otp" variants={fadeUp} initial="hidden" animate="visible" exit="exit" className="w-full max-w-md px-4 relative z-10 mt-auto md:mt-0 mb-4 md:mb-0">
              <div className="bg-white/70 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(113,0,0,0.1)] border border-white p-6 md:p-10 text-center relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-[#710000]/40 to-transparent" />
                
                <div className="w-12 h-12 md:w-16 md:h-16 bg-[#710000]/5 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4 border border-[#710000]/10">
                  <ShieldCheck className="text-[#710000] w-6 h-6 md:w-8 md:h-8" />
                </div>
                <h2 className="font-serif text-2xl md:text-3xl font-bold mb-1.5 md:mb-2">Verify OTP</h2>
                <p className="text-gray-500 mb-6 md:mb-8 font-medium text-xs md:text-sm">Sent to +91 {phoneNumber}</p>
                  ...
                  <div className="flex gap-3 md:gap-4 justify-center">
                    {otp.map((digit, index) => (
                      <input key={index} ref={el => { inputRefs.current[index] = el; }} type="text" maxLength={1} value={digit}
                        ...
                        className="w-12 h-14 md:w-14 md:h-16 text-center text-2xl md:text-3xl font-black bg-white border-2 border-gray-100 rounded-2xl focus:border-[#710000] focus:ring-4 focus:ring-[#710000]/10 outline-none shadow-sm transition-all text-[#710000]" />
                    ))}
                  </div>
                  {error && <p className="text-red-500 text-xs md:text-sm font-bold bg-red-50 py-2 rounded-lg">{error}</p>}
                  
                  <div className="text-center text-xs md:text-sm font-medium">
                    {canResend ? (
                      <button type="button" onClick={resendOtp} className="text-[#710000] font-bold underline hover:text-[#520000] transition-colors">
                        Didn't receive code? Resend OTP
                      </button>
                    ) : (
                      <span className="text-gray-400">
                        Didn't receive code? Resend in <span className="font-bold text-[#710000]">0:{otpTimer.toString().padStart(2, '0')}</span>
                      </span>
                    )}
                  </div>

                  <button type="submit" disabled={loading || otp.join('').length !== 4} className="w-full bg-gradient-to-r from-[#710000] to-[#520000] text-cream font-bold py-3.5 md:py-4 rounded-2xl hover:shadow-lg transition-all border border-[#910000]">
            ...
            <motion.div key="success" variants={fadeUp} initial="hidden" animate="visible" className="w-full max-w-md px-4 relative z-10 pt-12">
              ...
              >
                ...
                <div className="flex justify-between items-start relative z-10">
                  <div>
                    <p className="text-[9px] md:text-[10px] text-gold tracking-[0.15em] font-bold uppercase mb-0.5 md:mb-1">POP O'BOB FESTIVE</p>
                    <h3 className="font-serif text-3xl md:text-4xl font-medium text-cream tracking-wide">Gift Card</h3>
                    <p className="inline-block px-2.5 py-0.5 md:px-3 md:py-1 bg-[#3a0000]/60 text-gold border border-gold/40 text-[9px] md:text-[10px] font-bold tracking-[0.15em] uppercase rounded-full mt-1.5 md:mt-2 shadow-sm backdrop-blur-sm">Rakhi Edition</p>
                  </div>
                  <Gift className="text-gold w-6 h-6 md:w-8 md:h-8" strokeWidth={1.5} />
                </div>
                
                {/* Middle Section: Recipient */}
                <div className="relative z-10">
                  <p className="text-cream/70 text-xs md:text-sm mb-0.5 md:mb-1">For</p>
                  <p className="font-serif text-[24px] md:text-[28px] font-bold text-white tracking-wide leading-none">
                ...
                <div className="flex justify-end items-end relative z-10 w-full mt-2">
                  <div className="flex flex-col items-end gap-1.5 md:gap-2">
                    <div className="text-right bg-black/40 px-4 md:px-5 py-2.5 md:py-3 rounded-2xl backdrop-blur-md border border-white/10 w-full shadow-2xl">
                      <p className="text-cream/80 text-[10px] md:text-[11px] tracking-[0.25em] uppercase mb-1 md:mb-1.5 font-bold">CARD CODE</p>
                      <p className="font-mono text-[18px] md:text-[22px] font-bold text-white tracking-widest leading-none drop-shadow-md">
                        {bogoCode || 'BOGO-XXXX'}
                      </p>
                    </div>
                    <p className="text-gold text-[7px] md:text-[8px] lg:text-[9px] uppercase tracking-[0.15em] font-bold mr-2 text-right">Offer valid from 25th to 31st Aug</p>
                  </div>
                </div>
              </div>
